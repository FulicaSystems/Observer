...
class HostResourceABC : public ResourceABC
{
  ...
  public:
    ...
    HostResourceABC(const uint64_t index, const ResourceLoadInfoI *loadInfo) : index(index), loadInfo(loadInfo)
    {
    }
...
class LocalResourceABC : public ResourceABC
{
  ...
  public:
    ...
    LocalResourceABC(const class LogicalDevice &device, const HostResourceABC *host) : device(device), host(host)
    {
    }
